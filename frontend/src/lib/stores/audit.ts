...
function createAuditStore() {
	...
	async function submit(jdText: string, cvText: string): Promise<AuditAnonymousResult | null> {
		if (jdText.trim().length < 50) {
			store.set({ status: 'error', result: null, error: 'JD_TOO_SHORT', retryAfter: null });
			return null;
		}
		store.set({ status: 'loading', result: null, error: null, retryAfter: null });
		try {
			const result = await apiClient.auditAnonymous({
				jd_text: jdText.trim(),
				cv_text: cvText.trim() || undefined
			});
			store.set({ status: 'done', result, error: null, retryAfter: null });
			return result;
		} catch (err) {
			...
			store.set({ status: 'error', result: null, error: message, retryAfter });
			return null;
		}
	}
