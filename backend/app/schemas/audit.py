...
class AuditIssueResponse(BaseModel):
    """A problematic area detected in a CV-only audit."""
    ...
    severidad: str = Field(..., description="Issue severity: low, medium, high")
...
class AuditAnonymousResponse(BaseModel):
    ...
    mode: Literal["cv_only", "jd_directed"] = Field(
        "jd_directed", description="Which audit ran: cv_only (no JD) or jd_directed"
    )
    ...
    problematicas: list[AuditIssueResponse] = Field(
        default_factory=list,
        description="CV-only mode: detected problems with severity",
    )
    recomendaciones: list[str] = Field(
        default_factory=list,
        description="CV-only mode: actionable recommendations",
    )
